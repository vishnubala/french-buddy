...
async function main() {
  ...
  /* emit the map the app consumes (fetched at runtime as audio/clips.json) */
  const map = Object.fromEntries(clips.map((c) => [c.key, `audio/${c.key}.mp3`]));
  await writeFile(CLIP_MAP_OUT, JSON.stringify(map, null, 2));
  await writeFile(
    CLIP_MAP_OUT,
    `/* generated by generate-audio.mjs — do not edit by hand */\n` +
    `window.AUDIO_CLIPS = ${JSON.stringify(map, null, 2)};\n`
  );
